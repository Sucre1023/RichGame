...
public:
    ...
    void getpath(Sprite *playerSprite,int stepsCount,bool**canpassgrid,int HangInGrid,int LieInGrid );//stepscount:要走的步数 canpassgrid:地图能否走动的二维数组  HangInGrid:canpassgird数组的行数  LieInGrid:数组的列数
...
private:
    ...
    CC_SYNTHESIZE(std::vector<int>, pathlie_vector, pahtlie_vector);
protected:
    GetWalkPath(void);
    ~GetWalkPath(void);
